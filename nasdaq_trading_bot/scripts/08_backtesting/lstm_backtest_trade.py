"""
LSTM Backtest with Trading Logic (Entry/Exit Simulation)
========================================================
Simulates trades based on LSTM predictions, using the same logic as deployment.
"""
...
def run_simulation(days):
    ...
    model.load_state_dict(torch.load(os.path.join(MODELS_DIR, "best_lstm_model.pth"), map_location=DEVICE))
    model.eval()
    ...
    for col in feature_list:
        if col in df_feat.columns:
            ...
            # If missing, fill with zeros (neutral)
    ...
    if missing_cols:
        ...
        print(f"[DEBUG] Filled missing with 0.0")
        
    X_raw = np.column_stack(X_raw).astype(np.float32)
    
    # Scale X
    # Using found_cols is wrong if we filled missing ones. We should use feature_list.
    X_df = pd.DataFrame(X_raw, columns=feature_list)

    X_scaled = scaler_x.transform(X_df)
    
    # Prices for simulation
    # We need valid alignment. 
    # Logic: At index i, we have X[i-SEQ+1 : i+1]. This predicts for t+1..t+15.
    # Trade decision made at close of i. Fill at Open of i+1.
    ...
    for i in range(SEQ_LEN, len(df_feat) - 1):
        ...
        # We need next bar timestamp and open price
        t_next = df_feat.index[i+1]
        price_next_open = open_prices.iloc[i+1]
        
        # First, check exits for existing position using Open[i+1]
        # (Conservative: Check gap moves on open)
        if sim.position:
            # Check if we should have exited based on NEW info or stop hit on gap
            sim.try_exit(t_next, price_next_open, s, r3)
            
        # If still no position, try entry
        # Note: If we just exited, we usually wait? Deployment has cooldown.
        # Let's simple try_enter immediately if valid
    ...
    if sim.equity_curve:
        ...
        plt.title(f"LSTM Backtest Equity Curve ({dataset_name(days)})")
        plt.legend()
        plt.grid(True)
        out_path = os.path.join(PROJECT_ROOT, "images", "08_lstm_trade_backtest.png")
